...
const SubmitTask: FunctionalComponent = () => {
  ...
  console.log(
    "@@@@@  ~ file: index.tsx ~ line 15 ~ currentQuestion",
    currentQuestion
  );
  ...
  console.log(
    "@@@@@  ~ file: index.tsx ~ line 17 ~ surveyInFocus",
    taskInFocus
  );
  ...
  async function onSurveyCompleted() {
    ...
    const res = await Promise.all(
      questionIds.map(async (questionId) => {
        ...
        console.log(
          "@@@@@  ~ file: index.tsx ~ line 75 ~ questionIds.map ~ answer",
          answer
        );
    ...
    const incorrectResponseIds = res
      .filter((obj) => !obj.is_correct)
      .map((obj) => obj.details.question);
    console.log(
      "@@@@@  ~ file: index.tsx ~ line 110 ~ onSurveyCompleted ~ incorrectResponseIds",
      incorrectResponseIds
    );

    if (incorrectResponseIds.length > 0) {
      const incorrectQuestions = taskInFocus.questions.filter((q) =>
        incorrectResponseIds.some((id) => id === q.id)
      );
      console.log(
        "@@@@@  ~ file: index.tsx ~ line 113 ~ onSurveyCompleted ~ incorrectQuestions",
        incorrectQuestions
      );
    ...
    window.alert(
      "Congratulations you have successfully completed this activity and earned Zlto"
    );
  ...
  const percentCompleted =
    ((currentQuestion + 1) / questionsToShow.length) * 100;
            ...
            <Fragment>
              {currentQuestion === idx && (
                <div class={style.questionContainer}>
                  ...
                  {!!obj.sub_title && <span>{obj.sub_title}</span>}
                  {!!obj.description && <Description />}
                  {!!obj.question_attachments &&
                    !!obj.question_attachments.length && <Attachments />}
                  <div class={style.answerContainer}>
                    ...
                    {["Closed-ended", "Multiple Choice"].includes(
                      obj.question_type_verbose
                    ) && (
                    ...
                    {["Open-ended", "Short answer", "Paragraph"].includes(
                      obj.question_type_verbose
                    ) && (
                      <input
                        value={answers[obj.id]}
